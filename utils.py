...
def sync_order_to_finance(order_row: dict, finance_data: dict) -> bool:
    ...
    Simplified: only 💚 (fully paid) generates income. 🧡 and 🔴 do not.
    """
    ...
    # Only fully-paid orders (💚) generate income
    if payment_status == "💚":
    ...
    if balance_due > 0:
        new_transaction = {
            "id": tx_id,
            "name": f"הזמנה #{order_id} - \"{customer_name}\" [💚 תשלום]",
            "amount": float(balance_due),
            "Type": "Income",
            "Item": str(order_row.get("Item", "כללי")),
            "date": today_str,
            "order_id": order_id,
            "payment_type": "תשלום סופי",
            "is_automated": True,
            "icon": "💚"
        }
        ...
        new_transaction = {
            "id": tx_id,
            "name": f"הזמנה #{order_id} - \"{customer_name}\" [↩️ קיזוז]",
            "amount": float(balance_due),
            "Type": "Income",
            "Item": str(order_row.get("Item", "כללי")),
            "date": today_str,
